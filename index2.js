...
// let participantes = participantesJSON.usuarios;
// console.log('JSON DE PARTICIPANTES', participantes)
let nombreUser = "";

const usuarios = async () => {
    return new Promise((res, rej) => {
        fs.readFile("public/assets/participantes.json", "utf8", (err, data) => {
            err ? rej('Error') : res(JSON.parse(data));
        });
    });
}
...
app.use('/abracadabra/juego/:usuario', async (req, res, next) => {
    ...
    participantes = await usuarios();
    nombreUser = nombre;
    participantes.usuarios.includes(nombre) ? next() : res.status(404).send(`<div align="center"><img src="/img/who.jpeg" alt="Usuario Desconocido." /><br><b><h2>${nombre}, no es usuario.</b></h2></div>`);
});
